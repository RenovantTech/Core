...
class Container {
	...
	public const YAML_OBJ_SKELETON = [
	 ...
	 * @internal
	 */
	public function build(string $id, string $class, array $args = [], array $properties = []): object {
	 ...
	 * @throws \ReflectionException
	 */
	public function get(string $id, ?string $class = null, int $failureMode = self::FAILURE_EXCEPTION): object|null {
	 ...
	 */
	public function getListByType(string $class): array {
	...
	public function getType(string $id): string {
		if (isset($this->id2classMap[$id])) {
			return $this->id2classMap[$id][0];
		}
		throw new ContainerException(1, [$this->_, $id]);
	}
