...
use renovant\core\auth\{AuthException, AuthService};
...
class PdoProvider implements ProviderInterface {
	...
	public const SQL_AUTHENTICATE = 'SELECT %s FROM %s WHERE id = :id';
	public const SQL_CHECK_PWD    = 'SELECT %s FROM %s WHERE user_id = :user_id';

	public const SQL_TOKEN_CHECK  = 'SELECT COUNT(*) FROM `%s` WHERE type = :type AND user_id = :user_id AND token = :token AND expire >= NOW()';
	public const SQL_TOKEN_FETCH  = 'SELECT user_id, data FROM `%s` WHERE type = :type AND token = :token AND expire >= NOW()';
	public const SQL_TOKEN_SET    = 'INSERT INTO `%s` (type, user_id, token, data, expire) VALUES (:type, :user_id, :token, :data, FROM_UNIXTIME(:expire))';
	public const SQL_TOKEN_DELETE = 'DELETE FROM `%s` WHERE type = :type AND user_id = :user_id AND token = :token';

	public const SQL_2FA_DISABLE    = 'UPDATE `%s` SET tfaKey = NULL, tfaRescue = NULL WHERE user_id = :user_id';
	public const SQL_2FA_FETCH      = 'SELECT tfaKey, tfaRescue FROM `%s` WHERE user_id = :user_id';
	public const SQL_2FA_IS_ENABLED = 'SELECT COUNT(*) FROM `%s` WHERE user_id = :user_id AND tfaKey IS NOT NULL';
	public const SQL_2FA_SET        = 'UPDATE `%s` SET tfaKey = :tfaKey, tfaRescue = :tfaRescue WHERE user_id = :user_id';

	public const SQL_LOGIN        = 'SELECT user_id, active, password, tfaKey, tfaRescue FROM `%s` WHERE login = :login';
	public const SQL_SET_ACTIVE   = 'UPDATE `%s` SET active = :active WHERE user_id = :user_id';
	public const SQL_SET_EMAIL    = 'UPDATE `%s` SET email = :email WHERE id = :user_id';
	public const SQL_SET_PASSWORD = 'UPDATE `%s` SET password = :password, passwordExpire = FROM_UNIXTIME(:expire) WHERE user_id = :user_id';
	...
	/** @throws \SodiumException */
	public function fetch2FA(int $userID): array {
		...
		try {
			if ($currPwd) {
				$storedPwd = sys::pdo($this->pdo)->prepare(sprintf(self::SQL_CHECK_PWD, 'password', $this->tables['auth']))
					->execute(['user_id' => $userID])->fetchColumn();
				if (!password_verify($currPwd, $storedPwd)) {
					return AuthService::SET_PWD_CURR_INVALID;
				}
			}
