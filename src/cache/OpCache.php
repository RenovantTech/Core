...
class OpCache implements CacheInterface {
	 ...
	 */
	public function __construct(string $id, ?string $pdo = null, string $table = 'cache', bool $writeBuffer = false) {
		$this->id          = $id;
	...
	public function get(string $id) {
		if (isset($this->cache[$id])) {
			...
			if (file_exists($file = self::_file($this->id, $id))) {
				include $file;
			}
	...
	protected static function _file($cacheId, $id) {
		return CACHE_DIR . 'opc-' . $cacheId . '/' . substr_replace(substr_replace(md5($id), '/', 3, 0), '/', 7, 0);
	}

	protected static function _write($cacheId, $id, $value, $expire) {
