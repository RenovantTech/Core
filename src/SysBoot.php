...
class SysBoot extends sys {
	public const SYS_CACHE_DEFAULT_CONFIG = [
	...
	public static function boot() {
		...
		// directories
		if (!defined(__NAMESPACE__ . '\PUBLIC_DIR') && PHP_SAPI != 'cli') {
			die(SysException::ERR21);
		}
		...
		foreach ($config['sys']['namespaces'] as $k => $dir) {
			...
			if (substr($dir, 0, 7) == 'phar://') {
				if ($dir[7] != '/') {
					$dir = 'phar://' . BASE_DIR . substr($dir, 7);
				}
		...
		// Log service
		if (is_array($config['sys']['log'])) {
			self::$Sys->cnfLog = $config['sys']['log'];
		}
