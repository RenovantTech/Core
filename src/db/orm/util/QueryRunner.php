...
use renovant\core\db\{Procedure, Query};
use renovant\core\db\orm\{EntityTrait, Repository};
...
class QueryRunner {
	...
	public function __construct(?string $pdo, Metadata $Metadata) {
	...
	public function deleteOne(object $Entity, ?string $criteriaExp = null): bool {
	...
	public function deleteAll(?int $limit, ?string $orderExp, ?string $criteriaExp): int {
		$Query = (new Query($this->Metadata->sql('target'), null, $this->pdo))
			->setCriteriaDictionary($this->Metadata->criteria())
			->setOrderByDictionary($this->Metadata->fetchOrderBy())
			->orderByExp($orderExp)
	...
	public function fetchOne(string $class, ?int $offset, ?string $orderExp, ?string $criteriaExp, int $fetchMode = Repository::FETCH_OBJ, ?string $fetchSubset = null): object|array|false {
	...
	public function fetchAll(string $class, ?int $offset, ?int $limit, ?string $orderExp, ?string $criteriaExp, int $fetchMode = Repository::FETCH_OBJ, ?string $fetchSubset = null): array {
		$subset = ($fetchSubset) ? $this->Metadata->fetchSubset($fetchSubset) : '*';
		$Query  = (new Query($this->Metadata->sql('source'), $subset, $this->pdo))
			->setCriteriaDictionary($this->Metadata->criteria())
			->setOrderByDictionary($this->Metadata->fetchOrderBy())
	...
	public function update(object $Entity, array $changes, ?string $criteriaExp): bool {
		/** @var object|EntityTrait $Entity */
