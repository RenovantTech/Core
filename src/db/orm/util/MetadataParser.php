...
class MetadataParser {
	 ...
	 * @throws \Exception
	 */
	public static function parse(string $entityClass): array {
		$RefClass = new ReflectionClass($entityClass);

		// SQL source / target / functions
		$sql        = [];
		$DocComment = $RefClass->getDocComment();
		if (!$tags = $DocComment->getTagValues('orm')) {
			throw new Exception(602, [$entityClass]);
		}
		$tag = [];
		foreach ($tags as $t) {
			$tag = array_merge($tag, $t);
		}
		if (isset($tag['source'])) {
		...
		foreach ($sql as $k => $v) {
			if (is_null($v)) {
				throw new Exception(603, [$entityClass, $k]);
			}
		}
		...
		if ($DocComment->hasTag('orm-validate-subset')) {
			...
			foreach ($tagValues as $value) {
				foreach ($value as $k => $v) {
					$validateSubsets[$k] = trim(str_replace(' ', '', $v));
				}
			}
		}
		...
		foreach ($RefClass->getProperties() as $RefProperty) {
			...
			if (!$DocComment->hasTag('orm')) {
				continue;
			}
			$properties[$prop] = ['type' => 'string', 'null' => false, 'readonly' => false];
			if ($tag = $DocComment->getTag('orm')) {
				if (isset($tag['type']) && !in_array($tag['type'], ['string', 'integer', 'float', 'boolean', 'date', 'time', 'datetime', 'microdatetime', 'object', 'array'])) {
					throw new Exception(604, [$entityClass, $prop, $tag['type']]);
				}
				$properties[$prop] = array_merge($properties[$prop], (array)$tag);
				if (isset($tag['primarykey'])) {
					$pKeys[] = $prop;
				}
			}
		}
