...
use renovant\core\util\reflection\{ReflectionClass, ReflectionMethod};
...
class ActionControllerReflection {
	...
	public static function analyzeActions(ActionController $Controller) {
		...
		foreach ($refMethods as $RefMethod) {
			...
			if (fnmatch('renovant\core\*', $methodClass, FNM_NOESCAPE) && !$RefMethod->getDocComment()->hasTag('routing')) {
				continue;
			}
			// check signature of preHandle & postHandle hooks
			if (in_array($methodName, ['preHandle', 'postHandle'])) {
				if (!$RefMethod->isProtected()) {
					throw new Exception(101, [$methodClass, $methodName]);
				}
				...
				if ($DocComment->hasTag('routing')) {
					...
					if (isset($tag['pattern'])) {
						...
						$config[$action]['pattern'] = '/' . $action . '/';
					}
				} else {
					$config[$action]['method'] = '*';
					if ($action == constant(get_class($Controller) . '::DEFAULT_ACTION')) {
