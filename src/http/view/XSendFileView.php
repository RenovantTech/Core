...
class XSendFileView implements ViewInterface {
	...
	public function render(Request $Req, Response $Res, $resource = null, array $options = null) {
		if (!defined('XSENDFILE_PATH')) {
			throw new Exception(261);
		}
		if (!defined('XSENDFILE_URL')) {
			throw new Exception(262);
		}
		if (!file_exists(XSENDFILE_PATH . $resource)) {
			throw new Exception(201, ['X-SendFile', XSENDFILE_PATH . $resource]);
		}
