...
use renovant\core\util\reflection\{DocComment, ReflectionClass};

/**
 * @internal
 */
class OrmTagsParser {
	public static function parse(string $entityClass): ?OrmAuthz {
		try {
			...
			if (empty($roles) && empty($perms) && empty($acls)) {
				return null;
			} else {
				return new OrmAuthz($entityClass, $allows, $roles, $perms, $acls, $op_roles, $op_perms, $op_acls);
			}
	...
	protected static function parseAllows(DocComment $DocComment, ?string $action): ?array {
		$allows = null;
	...
	protected static function parseRoles(DocComment $DocComment, ?string $action): array {
		$roles  = $op = null;
		$docTag = ($action == '_') ? '' : '-' . strtolower($action);
	...
	protected static function parsePermissions(DocComment $DocComment, ?string $action): array {
		$perms  = $op = null;
		$docTag = ($action == '_') ? '' : '-' . strtolower($action);
	...
	protected static function parseAcls(DocComment $DocComment, ?string $action): array {
		$acls   = $op = null;
		$docTag = ($action == '_') ? '' : '-' . strtolower($action);
