...
class OrmAuthz {
	...
	public function __construct(string $entityClass, ?array $allows, ?array $roles, ?array $perms, ?array $acls, ?array $op_roles, ?array $op_perms, ?array $op_acls) {
		$this->_        = $entityClass;
		$this->allows   = $allows;
		$this->roles    = $roles;
		$this->perms    = $perms;
	...
	public function check(string $action, OrmEvent $OrmEvent): void {
	...
	protected function checkRoles(Authz $Authz, array &$checked, ?string $action = null): void {
		...
		switch ($this->op_roles[$action]) {
			...
			default:
				foreach ($this->roles[$action] as $role) {
					if (!$Authz->role($role)) {
						throw new AuthzException($exCode, [$role, $this->_, $action]);
					} else {
						$checked['ROLES'][] = $role;
					}
				}
	...
	protected function checkPermissions(Authz $Authz, array &$checked, ?string $action = null): void {
		...
		switch ($this->op_perms[$action]) {
			...
			default:
				foreach ($this->perms[$action] as $perm) {
					if (!$Authz->permission($perm)) {
						throw new AuthzException($exCode, [$perm, $this->_, $action]);
					} else {
						$checked['PERMISSIONS'][] = $perm;
					}
				}
			...
			default:
				foreach ($this->acls[$action] as $aclKey => $aclProp) {
					if ($values = $Authz->aclValues($aclKey)) {
						...
						throw new AuthzException($exCode, [$aclKey, $this->_, $action]);
					}
				}
		}
	}
